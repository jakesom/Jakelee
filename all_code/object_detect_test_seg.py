...
def inference(weights_path, img_path, crop_size, overlap_size, savejson_func, json_output_dir, postfix,
    ...
    # 改舰船的标签名字   9月9日更改版
    if weights_path == 'weights/ship_seg.pt':
        wei = 'ship'
        ...
        wei = 'harbor_1017'
        names = {0: 'wharf'}
    elif weights_path == 'weights/best_oil.pt':
        wei = 'oiltank'
        names = {0: 'oiltank'}
    ...
    for i, x in enumerate(x_steps):
        for j, y in enumerate(y_steps):
            ...
            #水平框
            if weights_path == 'weights/harbor_1017.pt' or weights_path == 'weights/best_oil.pt' :
                if  len(result[0].boxes.data) > 0:
                    detect_yes = True
                        # 遍历检测框
    ...
    # nms_results = combined_results
        ...
        if weights_path == 'weights/best_oil.pt':
            ...
            oiltank_list = [item for item in nms_results if item['cls'] == 'oiltank']
            ...
            nms_results = [item for item in nms_results if item['cls'] != 'oiltank'] + filtered_oiltank_list
        ...
        for box in boxes:
            ...
            if isinstance(box["mask"][1], int):
                ...
                for mask_, x1_, y1_ in zip(box["mask"][0], box["mask"][1], box["mask"][2]):
                    ...
                    y1_ = y1_  # 将y调整为当前矩阵的y坐标
                    find_max_x = find_max_in_nested_list(x1_)
                    min_x = min(min_x, find_max_x)  # 更新最小x坐标
                    find_max_y = find_max_in_nested_list(x1_)
                    min_y = min(min_y, find_max_y)  # 更新最小y坐标
                ...
                for i, (masknn, x1nn, y1nn) in enumerate(zip(box["mask"][0], box["mask"][1], box["mask"][2])):
                    ...
                    x1nn = find_max_in_nested_list(x1nn)
                    y1nn = find_max_in_nested_list(y1nn)
...
def class_begin():
    ...
    if img_path_Port:
        # 检测 舰船
        Use_Mask = True
        weights_path = r'weights/ship_seg.pt'
        image_name = 'ship'
        conf_score = 0.35
        ...
        crop_size = (640, 640)
        for img_path in img_path_Port:
            weights_path = r'weights/harbor_1017.pt'
            img_path = convert_image_data(base64_to_image(img_path))
            big_img = big_pic_paste
            results, img, colorss, names, all_Port_results, weights_path, big_pic_paste, num_id = infrence(
                cut_pic_lefttop_point, big_img, img_path, weights_path, crop_size, overlap_size,
