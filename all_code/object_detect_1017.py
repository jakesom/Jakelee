...
def inference(weights_path, img_path, crop_size, overlap_size, savejson_func, json_output_dir, postfix,
              Use_Masks, ds_min_x, ds_max_y, geotransform, projection, cut_pic_lefttop_point, num_id, conf_score,detected_coordinates):
    ...
    if weights_path == 'weights/ship801.pt':
        names = {0: 'aircraft_carrier', 1: 'other_ship1', 2: 'cargo_ship', 3: 'other_ship2', 4: 'other_ship3',
         ...
         35: 'other_ship22',
         36: 'other_ship23', 37: 'aircraft_carrier', 38: 'patrol_ship', 39: 'other_ship25', 40: 'other_ship26',
    ...
    elif weights_path == 'weights/airport707.pt':
    ...
    # detect_yes = False
    if weights_path == 'weights/airport707.pt':
        for i, x in enumerate(x_steps):
            for j, y in enumerate(y_steps):
                ...
                if len(result[0].boxes.data) > 0:
                    detected_coordinates.append((i, j))
                # ----------------------------------保存有机场部件的切片------------------------------------------
    ...
    if weights_path == 'weights/airplane729.pt':
        for i, x in enumerate(x_steps):
            for j, y in enumerate(y_steps):
                ...
                if x + crop_size[0] > width:
                    x = width - crop_size[1]
                if (i, j) not in detected_coordinates:
                    continue  # 如果 (i, j) 在 detected_coordinates 里面，跳过这一轮
                cropped_image = img[y:y + crop_size[1], x:x + crop_size[0]]
                result = model.predict(cropped_image, imgsz=640, conf=conf_score)
                names = result[0].names
                # 起始坐标点
                ...
                if len(result[0].obb.data) > 0:
                    ...
                    # 遍历检测框
                    for det, xywhr, xyxy, xyxyxyxy in zip(result[0].obb.data, result[0].obb.xywhr,
                                                          result[0].obb.xyxy, result[0].obb.xyxyxyxy):
                        # 克隆张量，避免原地修改
                        problem = False
                        det = det.clone()
    ...
    if weights_path == 'weights/ship801.pt':
        for i, x in enumerate(x_steps):
            for j, y in enumerate(y_steps):
                ...
                if x + crop_size[0] > width:
                    x = width - crop_size[1]
                # if (i, j) in detected_coordinates:
                #     continue  # 如果 (i, j) 在 detected_coordinates 里面，跳过这一轮
                cropped_image = img[y:y + crop_size[1], x:x + crop_size[0]]
                result = model.predict(cropped_image, imgsz=640, conf=conf_score)
        ...
        for i, x in enumerate(x_steps):
            for j, y in enumerate(y_steps):
                ...
                if len(result[0].boxes.data) > 0:
                    ...
                    # if weights_path == 'weights/airplane729.pt' or weights_path == 'weights/harbor.pt':
    ...
    # nms_results = combined_results
        ...
        if weights_path == 'weights/best_oil.pt':
            ...
            filtered_oiltank_list = [oiltank_list[i] for i in valid_indices]
...
def infrence(cut_pic_lefttop_point, big_img, img_path, weights_path, crop_size, overlap_size,
             Use_Mask, ds_min_x, ds_max_y, geotransform, projection, big_img_path, image_name, num_id, conf_score,detected_coordinates):
    ...
    results, img, colorss, names, all_Port_results, weights_path, num_id = inference(weights_path, img_path, crop_size,
                                                                                     ...
                                                                                     conf_score,detected_coordinates)
...
def class_begin():
    ...
    if img_path_Airport:
        ...
        if img_path_Airport:
            image_name = 'Airport'
            weights_path = r"weights/airport707.pt"
            conf_score = 0.35
...
#油库港口分开测试版本;飞机、舰船为旋转框;先检测飞机场部件后再在此基础上检测飞机