# -*- coding: gbk -*-
"""
qq.tif可以跑旋转框，zy.tif不可以，没有检测出来的情况会报错
"""
import base64
import logging
import math
        ...
        for j, y in enumerate(y_steps):
            ...
            #旋转框
                ...
                if len(result[0].obb.data) > 0:
                    ...
                    # 遍历检测框
                    for det, xywhr, xyxy in zip(result[0].obb.data, result[0].obb.xywhr,result[0].obb.xyxy):
                        # 克隆张量，避免原地修改
                        problem = False
                        det = det.clone()
    ...
    if not Use_Masks:
        ...
        for box in boxes:
            ...
            if weights_path == 'weights/ship729.pt' or weights_path == 'weights/airplane729.pt':
                ...
                label = f" {clss}"
                # 在图像上画出框,绘制定向边界框
                cv2.drawContours(image, [box], 0, colors, 2)
                ...
                # x1_box ,y1_box = map(int, (x1_box, y1_box))
                ...
                colors = color[cls]
                # --------------------
                cv2.drawContours(big_img, [box], 0, colors, 2)
