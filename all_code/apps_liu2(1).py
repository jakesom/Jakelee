...
from image_transfer2 import draw_longitude
...
def inference(weights_path, img_path, crop_size, overlap_size, savejson_func, json_output_dir, postfix,
            ...
            if len(result[0].boxes.data) > 0:
                ...
                if weights_path == 'weights/airplane.pt' or weights_path == 'weights/harbor.pt':
                    ...
                    for i, det in enumerate(result[0].boxes.data):
                        ...
                        resultx = result
                        xywh = result[0].boxes.xywh[i].cpu().detach().numpy().tolist()
                        print('xywh:', xywh)
                        result_x, result_y, width_xywh, height_xywh = xywh

                        target_width = round(width_xywh, 3)
                        target_height = round(height_xywh, 3)
                        #-----------------------类别计数-----------------------
                        if not problem:
                            combined_results.append({
                                "initial_point": [x,y],
                                ...
                                'conf':conf,
                                # 'box': det,  # 检测框
                                'box_point': [x1_longitude_change,y1_longitude_change,x2_longitude_change,y2_longitude_change],  # 检测框
                    ...
                    for det, msk in zip(result[0].boxes.data, result[0].masks.data):
                        ...
                        combined_results.append({
                            ...
                            'conf':conf,
                            # 'box': det,  # 检测框
                            'box_point': [x1_longitude_change,y1_longitude_change,x2_longitude_change,y2_longitude_change],
                            'box_locenter_coordinatengitude': [t_x1,t_y1,t_x2,t_y2],
                            'mask': [msk, x, y],  # 掩码
                            'longitude': [str_longitude_x, str_longitude_y],  # 经纬度坐标
                            "num_id": num_id,
                            "cls_name_combinate": cls_name_combinate,
    ...
    #     'initial_point':xy
    # }
    # nms_results_convert.append(initial_point)
    for data in nms_results:
        selected_data = {
            'initial_point': data['initial_point'],
            'class': data['class'],
            'conf': data['conf'].item(),
            'box': data['box'].tolist(),
            'box_longitude': data['box_longitude'],
            'longitude': data['longitude'],
            'num_id': data['num_id'],
            'cls_name_combinate': data['cls_name_combinate'],
            'target_width':data['target_width'],
            'target_height':data['target_height'],
        }
        nms_results_convert.append(selected_data)

    print('nms_results_convert:',nms_results_convert)



    grouped_data = {}

    for data in nms_results_convert:

        initial_point = tuple(data['initial_point'])
        if initial_point not in grouped_data:
            grouped_data[initial_point] = []
        grouped_data[initial_point].append(data)
    count = 0
    # 遍历分组的数据，并保存到不同的 JSON 文件中
    for initial_point, group in grouped_data.items():
        json_filename = f"image_{count}.{postfix}"
        # 创建json文件路径
        json_file_path = os.path.join(json_output_dir, json_filename)
    ...
    # 保存json文件
    # with open(json_file_path, "w") as file:
    #     file.write(nms_results_convert)
    # tile_id增加1
    tile_id += 1
...
def savejson(results, file_path, names, all_results, json_filename):
    ...
    for i in range(len(results)):
        ...
        for j in range(len(data)):
            ...
            box_info = data.box_info[j]['box_point']
            num_id_info = data.num_info[j]['num_id']
            cls_name_combinate_info = data.cls_name_combinate_info[j]['cls_name_combinate']
            conf = round(data.boxes.conf[j].item(), 3)
            xywh = data.boxes.xywh[j].cpu().detach().numpy().tolist()
            corner = [
                [round(box_info[0].item(), 3), round(box_info[1].item(), 3)],  # 左上角
                [round(box_info[2].item(), 3), round(box_info[1].item(), 3)],  # 右上角
                [round(box_info[2].item(), 3), round(box_info[3].item(), 3)],  # 右下角
                [round(box_info[0].item(), 3), round(box_info[3].item(), 3)]  # 左下角
            ]
            center_coordinate = [round((box_info[0].item()+box_info[2].item())/2, 3), round((box_info[1].item()+box_info[3].item())/2, 3)]
            # corner = [
            ...
            lslist = {
                ...
                "conf": conf,
                # "corner": corner,
                "box_coordinate_LL": corner_LL,#4个点的经纬度坐标
                "box_coordinate": corner,
                "num_id": num_id_info,
                "cls_name_combinate_info": cls_name_combinate_info,
                "center_coordinate_LL": detection_info,#中心点的经纬度
        ...
        # print("查看all_results：", all_results)
        # # 用于计算两个坐标之间的距离
        ...
        #
        # def remove_similar_boxes(all_results):
        #     new_result = []
        ...
        #                 runway_annotations = [ann for ann in annotations if 'class' in ann]
        #                 for i, ann1 in enumerate(runway_annotations):
        #                     for j, ann2 in enumerate(runway_annotations[i + 1:]):
        #                         if 'box_coordinate' not in ann1 or 'box_coordinate' not in ann2:
        #                             continue
        #                         distance = calculate_distance(ann1['box_coordinate'][0], ann2['box_coordinate'][0])
        #                         if distance < 200:
        #                             remove_box_name = ann1.get("num_id")
        #                             similar_indexes.append(remove_box_name)
        #                             # similar_indexes.append(i + j + 1)
        #                             print("num_id:", remove_box_name)
        ...
        #                             continue
        #                         if item['num_id'] in similar_indexes:
        #                             del item
        #                         else:
        #                             filtered_items.append(item)
        #                     filtered_annotations.append({key: filtered_items})
        #                     new_result.append(filtered_annotations)
        #
        #     return new_result
        #
        # # 调用函数进行筛选
        # all_results = remove_similar_boxes(all_results)
        # print("剔除后的all_results:", all_results)
        #
