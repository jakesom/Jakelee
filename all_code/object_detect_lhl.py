...
def inference(weights_path, img_path, crop_size, overlap_size, savejson_func, json_output_dir, postfix,
    ...
    # 读取图像
    img = img_path
    # img =cv2.imread(r'D:\E\Python\flask_all\flask\harborpic1.jpg')
    ...
    if weights_path == 'weights/ship50.pt':
        ...
        names = {0: 'bomber', 1: 'transport_plane', 2: 'warplane', 3: 'transport_plane', 4: 'early_warning_airplane',
                 5: 'oil_plane', 6: 'antisubmarine_plane'}
    ...
    for i, x in enumerate(x_steps):
        for j, y in enumerate(y_steps):
            ...
            if len(result[0].boxes.data) > 0:
                ...
                if weights_path == 'weights/airplane.pt' or weights_path == 'weights/harbor_1017.pt':
                    # 遍历检测框
    ...
    # 进行非极大值抑制
    keep = nms(coords, scores, iou_threshold=0.3)
    # 获取非极大值抑制后的结果
    ...
    # def merge_runways_list(runways_list, thresholdx, thresholdy):
    #     merged_runways = []  # 用于存放合并后的跑道
    #     while len(runways_list) > 0:
    #         # 从列表中取出一条跑道
    #         runway1 = runways_list.pop(0)
    #         # 找出与当前跑道符合合并条件的跑道
    #         merge_candidates = [r for r in runways_list if (abs(r['box'][0] - runway1['box'][2]) < thresholdx
    #                                                         or abs(r['box'][2] - runway1['box'][0]) < thresholdx)
    #                                                         and  (abs(r['box'][3] - runway1['box'][1]) < thresholdy
    #                                                               or abs(r['box'][1] - runway1['box'][3]) < thresholdy)]
    #
    #         # 如果有符合条件的跑道，则进行合并
    #         if merge_candidates:
    #             # 初始化合并后的新跑道信息
    ...
    #             merged_indices = [runways_list.index(r) for r in merge_candidates]
    #             for idx, runway2 in zip(merged_indices, merge_candidates):
    #                 runway_to_merge = runways_list[idx]
    ...
    #                 merged_runway['xy_offset'] = min(merged_runway['xy_offset'], runway2['xy_offset'])
    #                 merged_runway['json_output_dir'] = runway1['json_output_dir']
    ...
    #
    #             # Replace merged runways in the original list
    #             merged_runway['mask'] = [
    #                 merged_mask[0],
    #                 merged_mask[1],
    #                 merged_mask[2]
    ...
    #             runways_list = [r for idx, r in enumerate(runways_list) if idx not in merged_indices]
    #         else:
    ...
    #     count = 0
    #     countj = 0
    #     lencount = len(runways_list)
    #     merged_mask = []
    #     # for i in range(len(runways_list)):
    #     while True:
    #         if count == lencount:
    #             break
    #         if len(runways_list) <= i:
    #             i = 0
    #         runway1 = runways_list[i]
    #         merged_runway = None  # Initialize merged_runway
    #         # for j in range(i + 1, len(runways_list)):
    #
    #         while True:
    #             if countj == lencount:
    #                 break
    #             if len(runways_list) <= j:
    #                 j = 0
    #             runway2 = runways_list[j]
    #             if (abs(runway2['box'][0] - runway1['box'][2]) < thresholdx
    #                 or abs(runway2['box'][2] - runway1['box'][0]) < thresholdx) \
    #                     and (abs(runway2['box'][3] - runway1['box'][1]) < thresholdy
    #                          or abs(runway2['box'][1] - runway1['box'][3]) < thresholdy):
    #
    #                 tensor1 = torch.tensor(min(runway1['box'][0], runway2['box'][0]), device='cuda:0')
    #                 tensor2 = torch.tensor(min(runway1['box'][1], runway2['box'][1]), device='cuda:0')
    #                 tensor3 = torch.tensor(max(runway1['box'][2], runway2['box'][2]), device='cuda:0')
    #                 tensor4 = torch.tensor(max(runway1['box'][3], runway2['box'][3]), device='cuda:0')
    #                 tensor5 = torch.tensor(max(runway1['conf'], runway2['conf']), device='cuda:0')
    #                 tensor6 = torch.tensor(3.0, device='cuda:0')
    #
    #                 stacked_tensor = torch.cat(
    #                     [tensor1.unsqueeze(0), tensor2.unsqueeze(0), tensor3.unsqueeze(0), tensor4.unsqueeze(0),
    ...
    #
    #                 merged_mask[0].append(runway2['mask'][0])
    #                 if type(runway2['mask'][1]) is list:
    #                     for n in runway2['mask'][1]:
    #                         merged_mask[1].append(n)
    #                 else:
    #                     merged_mask[1].append(runway2['mask'][1])
    #                 if type(runway2['mask'][2]) is list:
    #                     for n in runway2['mask'][2]:
    #                         merged_mask[2].append(n)
    #                 else:
    #                     merged_mask[2].append(runway2['mask'][2])
    #                 # merged_mask[2].append(runway2['mask'][2])
    #
    ...
    #
    #                 merged_runways.append(merged_runway)
    #                 # merged_runway[0].append(runway2['mask'][0])
    #                 # for item in
    #                 runways_list.append(merged_runway)
    #                 # runways_list.remove(runway2)
    #
    #                 # runways_list.remove(runway2)
    #             else:
    #                 merged_runways.append(runway1)  # Add original runway if no merge
    #
    #         runways_list.remove(runway1)
    #         i += 1
    #         count += 1
    #
    #     return runways_list
